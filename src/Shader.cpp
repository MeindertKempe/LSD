...
void Shader::DestroyShader() { glDeleteProgram(this->programID); }

void Shader::UseProgram() { glUseProgram(this->programID); }

u32 Shader::GetProgram() { return programID; }
...
std::string Shader::LoadShaderFromFile(std::string filePath) {
	...
	try {
		...
		std::cout << "ERROR::SHADER::FILE_NOT_SUCCESSFULLY_READ: " << e.what() << "IN::" << filePath
		          << std::endl;
	}
	return shaderCode;
}
